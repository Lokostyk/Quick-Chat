...
export default function LeftBar({singleConversations,groupConversations,setChosenChat}:{singleConversations:fetchedUser[],groupConversations:fetchedChatData[],setChosenChat:React.Dispatch<React.SetStateAction<{userOneId: string;userTwoId: string;}>>}) {
  ...
  return (
    ...
    <div className="leftBarContainer">
        <button className="mobileBtn" onClick={closeLeftBar}><img src="./Images/openLeftBar.svg" /></button>
        ...
        <div className="chatList">
          ...
          {singleConversations.map((item:fetchedUser)=>{
            return (<button key={item._id} className="chatElement" onClick={()=>openChat(item._id,state._id)} data-testid="singleTiles">
        ...
        <div className="chatList">
            <h2>Group Chats
              <button onClick={()=>setCreateGroup(true)} data-testid="addGroupBtn">
            ...
            {groupConversations.map(item=>{
              return (<button key={item._id} className="chatElement" onClick={()=>openChat(item._id)} data-testid="groupTiles">
                <p>{item.groupName}</p>
              </button>)
            })}
        </div>
        <div className="bottomInfo">
          <img src={state.imgSmall === ""?"/Images/default.jpg":state.imgSmall} data-testid="userImagePreview"/>
          <h2>{`${state.name} ${state.surname}`}</h2>
          <div className="settingsContainer" onClick={(e)=>e.stopPropagation()} data-testid="settingsWindow">
                ...
                <div className="themeConatiner">
                  <h2>Theme settings</h2>
                    <div style={{display:"inline-block"}}>
                      ...
                      <button className="themeBtn" onClick={handleThemeChange} data-id="2" data-testid="colorBtn"></button>
                      <button className="themeBtn" onClick={handleThemeChange} data-id="3"></button>
                    </div>
                </div>
                <button className="logOutBtn" onClick={handleLogOut}>Log Out</button>
              </div>
            <button className="settingsIcon" onClick={()=>openSettings()} data-testid="settingsButton">
