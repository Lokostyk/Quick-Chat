...
export default function ChatWindow({chatData,socket}:{chatData:fetchedChatData,socket:Pusher}) {
  ...
  const sendMessage = (e:React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if(/^\s*$/.test(currentMessage)) return
  ...
  <section className="chatSection">
      <div className="topBar">
        ...
        <p data-testid="chatName">{chatData.groupName?chatData.groupName:otherUsersData[0].name + " " + otherUsersData[0].surname}</p>
        <button className="settingsBtn" onClick={(e)=>openSettings(e)} data-testid="converastionSettingsButton">
